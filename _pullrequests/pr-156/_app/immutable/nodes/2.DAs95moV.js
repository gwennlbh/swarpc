import { H as HttpError } from "../chunks/kMd03mTr.js";
...
import { L } from "../chunks/D8HaC4jT.js";
...
function WorkerWrapper$1(options) {
  return new SharedWorker(
    "" + new URL("../workers/service-worker-C3-vIF0b.js", import.meta.url).href,
    {
      type: "module",
      name: options?.name
    }
...
function Client(procedures2, { worker, nodes: nodeCount, loglevel = "debug", restartListener = false, hooks = {}, localStorage = {}, nodeIds = [] } = {}) {
  ...
  for (const functionName of Object.keys(procedures2)) {
    ...
    const _broadcastProcedure = async ({ input, onProgresses, nodesCountOrIDs, concurrencyKey }) => {
      ...
      const _extras = {
        byNode: new Map(settleds.map(({ node: node2, ...result }) => [node2, result])),
        successes: sizedArray(extractFulfilleds(settleds).map((r) => r.value)),
        failures: sizedArray(extractRejecteds(settleds)),
        get failureSummary() {
          return this.failures?.map(({ node: node2, reason }) => `Node ${node2}: ${reason}`).join(";\n");
        },
        get ok() {
