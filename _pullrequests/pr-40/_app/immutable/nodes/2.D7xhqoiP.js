...
let ArkErrors$1 = class ArkErrors extends ReadonlyArray$1 {
  ...
  _add(error2) {
    ...
    if (existing) {
      if (existing.hasCode("union") && existing.errors.length === 0)
        return;
      const errorIntersection = error2.hasCode("union") && error2.errors.length === 0 ? error2 : new ArkError$1({
        code: "intersection",
  ...
  addAncestorPaths(error2) {
    for (const propString of error2.path.stringifyAncestors()) {
      this.byAncestorPath[propString] = append$1(this.byAncestorPath[propString], error2);
    }
  }
};
let TraversalError$1 = class TraversalError extends Error {
  ...
  constructor(errors) {
    if (errors.length === 1)
      ...
      super("\n" + errors.map((error2) => `  • ${indent$1(error2)}`).join("\n"));
...
const indent$1 = (error2) => error2.toString().split("\n").join("\n  ");
let Traversal$1 = class Traversal {
  ...
  errorFromContext(errCtx) {
    const error2 = new ArkError$1(errCtx, this);
    if (this.currentBranch)
...
const writeJsonSyntaxErrorProblem$1 = (error2) => {
  if (!(error2 instanceof SyntaxError))
    throw error2;
  return `must be ${jsonStringDescription$1} (${error2})`;
};
...
const procedures = {
  multiply: {
    input: type$1({ a: "number", b: "number" }),
    progress: type$1({ progress: "0 <= number <= 1", node: "string" }),
    success: type$1({ result: "number", node: "string" })
  }
};
function WorkerWrapper$1(options) {
  return new SharedWorker(
    "" + new URL("../workers/service-worker-CTP1cIU9.js", import.meta.url).href,
    {
      type: "module",
      name: options?.name
    }
...
class ArkErrors2 extends ReadonlyArray {
  ...
  _add(error2) {
    ...
    if (existing) {
      if (existing.hasCode("union") && existing.errors.length === 0)
        return;
      const errorIntersection = error2.hasCode("union") && error2.errors.length === 0 ? error2 : new ArkError2({
        code: "intersection",
        errors: existing.hasCode("intersection") ? [...existing.errors, error2] : [existing, error2]
      }, this.ctx);
      const existingIndex = this.indexOf(existing);
...
class Traversal2 {
  ...
  errorFromContext(errCtx) {
    const error2 = new ArkError2(errCtx, this);
    if (this.currentBranch)
      this.currentBranch.error = error2;
    else
      this.errors.add(error2);
...
function Client(procedures2, { worker, nodes: nodeCount, loglevel = "debug", restartListener = false, hooks = {}, localStorage = {} } = {}) {
  ...
  for (const functionName of Object.keys(procedures2)) {
    ...
    const _runProcedure = async (input, onProgress = () => {
      ...
      const l2 = createLogger("client", loglevel, nodeId ?? "(SW)", requestId);
    ...
    instance[functionName].broadcast = async (input, onProgress, nodesCount) => {
      ...
      const results = await Promise.allSettled(nodesToUse.map(async (id) => _runProcedure(input, onProgress, void 0, id)));
