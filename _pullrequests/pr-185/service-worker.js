...
function injectIntoConsoleGlobal(scope, nodeId, requestId) {
	for (const method of PATCHABLE_LOG_METHODS) scope.self.console[method] = (...args) => logger(method, "server", nodeId)(requestId, ...args);
}
//#endregion
//#region node_modules/swarpc/dist/scopes.js
var MockedWorkerGlobalScope = class {
	constructor() {}
};
...
function nodeIdFromScope(scope, _scopeType) {
	if (scopeIsDedicated(scope, _scopeType) || scopeIsShared(scope, _scopeType)) return scope.name;
	return "(SW)";
}
//#endregion
...
//#endregion
//#region node_modules/swarpc/dist/payload.js
...
function isPayloadHeader(procedures, payload) {
	if (typeof payload !== "object") return false;
	if (payload === null) return false;
	if (!("by" in payload)) return false;
...
function validatePayloadCore(procedure, payload) {
	...
	if ("abort" in payload && typeof payload.abort === "object" && payload.abort !== null && "reason" in payload.abort && typeof payload.abort.reason === "string") return { abort: { reason: payload.abort.reason } };
	if ("error" in payload && typeof payload.error === "object" && payload.error !== null && "message" in payload.error && typeof payload.error.message === "string") return { error: { message: payload.error.message } };
	throw new Error("invalid payload");
}
//#endregion
//#region node_modules/swarpc/dist/localstorage.js
var FauxLocalStorage = class {
	data;
	keysOrder;
	constructor(data) {
		this.data = data;
...
//#endregion
//#region node_modules/swarpc/dist/server.js
var abortControllers = /* @__PURE__ */ new Map();
var abortedRequests = /* @__PURE__ */ new Set();
function Server(procedures, { loglevel = "debug", scope, _scopeType } = {}) {
	scope ??= self;
	...
	for (const functionName in procedures) instance[functionName] = ((implementation) => {
		if (!instance[zProcedures][functionName]) throw new Error(`No procedure found for function name: ${functionName}`);
	...
	instance.start = async () => {
		const port = await new Promise((resolve) => {
			if (!scopeIsShared(scope, _scopeType)) return resolve(void 0);
		...
		const postMessage = async (autotransfer, data) => {
			...
			if (port) port.postMessage(data, { transfer });
			else if (scopeIsDedicated(scope, _scopeType)) scope.postMessage(data, { transfer });
			else if (scopeIsService(scope, _scopeType)) await scope.clients.matchAll().then((clients) => {
		...
		const listener = async (event) => {
			...
			const postError = async (error) => postMsg({ error: { message: "message" in error ? error.message : String(error) } });
			...
			if ("isInitializeRequest" in payload) throw "Unreachable: #initialize request payload should've been handled already";
			if ("abort" in payload) {
				const controller = abortControllers.get(requestId);
				if (!controller) await postError("No abort controller found for request");
		...
		if (scopeIsShared(scope, _scopeType)) {
			...
			port.start();
		} else if (scopeIsDedicated(scope, _scopeType)) scope.addEventListener("message", listener);
		else if (scopeIsService(scope, _scopeType)) scope.addEventListener("message", listener);
		else throw new Error(`Unsupported worker scope ${scope}`);
	};
	return instance;
}
//#endregion
...
var stringAndSymbolicEntriesOf = (o) => [...Object.entries(o), ...Object.getOwnPropertySymbols(o).map((k) => [k, o[k]])];
...
var capitalize$1 = (s) => s[0].toUpperCase() + s.slice(1);
...
var tryParseWellFormedNumber = (token, options) => parseNumeric(token, "number", {
	...options,
	strict: true
});
...
var typePathToPropString = (path) => stringifyPath(path, { stringifyNonKey: (node) => node.expression });
var referenceMatcher = /"(\$ark\.[^"]+)"/g;
var compileMeta = (metaJson) => JSON.stringify(metaJson).replace(referenceMatcher, "$1");
var flatRef = (path, node) => ({
	path,
	node,
	propString: typePathToPropString(path)
});
var flatRefsAreEqual = (l, r) => l.propString === r.propString && l.node.equals(r.node);
var appendUniqueFlatRefs = (existing, refs) => appendUnique(existing, refs, { isEqual: flatRefsAreEqual });
var appendUniqueNodes = (existing, refs) => appendUnique(existing, refs, { isEqual: (l, r) => l.equals(r) });
//#endregion
...
var implementation$21 = implementNode({
	kind: "predicate",
...
var implementation$20 = implementNode({
	kind: "divisor",
	collapsibleKey: "rule",
...
var parseExclusiveKey = { parse: (flag) => flag || void 0 };
...
var implementation$19 = implementNode({
	kind: "after",
	collapsibleKey: "rule",
	hasAssociatedError: true,
	keys: { rule: {
...
var implementation$18 = implementNode({
	kind: "before",
	collapsibleKey: "rule",
	hasAssociatedError: true,
	keys: { rule: {
...
var implementation$17 = implementNode({
	kind: "exactLength",
	collapsibleKey: "rule",
...
var implementation$16 = implementNode({
	kind: "max",
	collapsibleKey: "rule",
	hasAssociatedError: true,
	keys: {
...
var implementation$15 = implementNode({
	kind: "maxLength",
	collapsibleKey: "rule",
	hasAssociatedError: true,
...
var implementation$14 = implementNode({
	kind: "min",
	collapsibleKey: "rule",
	hasAssociatedError: true,
	keys: {
...
var implementation$13 = implementNode({
	kind: "minLength",
	collapsibleKey: "rule",
	hasAssociatedError: true,
...
var implementation$12 = implementNode({
	kind: "pattern",
	collapsibleKey: "rule",
	keys: {
		rule: {},
...
var implementation$11 = implementNode({
	kind: "optional",
...
var supportedJsonSchemaTargets = ["draft-2020-12", "draft-07"];
...
var exclusivizeRangeSchema = (schema) => typeof schema === "object" && !(schema instanceof Date) ? {
	...schema,
	exclusive: true
} : {
	rule: schema,
...
var defineRightwardIntersections = (kind, implementation) => flatMorph(schemaKindsRightOf(kind), (i, kind) => [kind, implementation]);
//#endregion
...
var implementation$10 = implementNode({
	kind: "alias",
	hasAssociatedError: false,
...
var implementation$9 = implementNode({
	kind: "domain",
	hasAssociatedError: true,
...
var implementation$8 = implementNode({
	kind: "intersection",
	hasAssociatedError: true,
	normalize: (rawSchema) => {
...
var implementation$7 = implementNode({
	kind: "morph",
	hasAssociatedError: false,
	keys: {
		in: {
...
var implementation$6 = implementNode({
	kind: "proto",
	hasAssociatedError: true,
...
var implementation$5 = implementNode({
	kind: "union",
	hasAssociatedError: true,
...
var implementation$4 = implementNode({
	kind: "unit",
	hasAssociatedError: true,
	keys: { unit: {
...
var implementation$3 = implementNode({
	kind: "index",
	hasAssociatedError: false,
	intersectionIsOpen: true,
	keys: {
...
var implementation$2 = implementNode({
	kind: "required",
	hasAssociatedError: true,
	intersectionIsOpen: true,
	keys: {
...
var implementation$1 = implementNode({
	kind: "sequence",
	hasAssociatedError: false,
...
var bindModule = (module, $) => new RootModule(flatMorph(module, (alias, value) => [alias, hasArkKind(value, "module") ? bindModule(value, $) : $.bindReference(value)]));
//#endregion
//#region node_modules/@ark/schema/out/scope.js
var schemaBranchesOf = (schema) => isArray(schema) ? schema : "branches" in schema && isArray(schema.branches) ? schema.branches : void 0;
...
var resolutionsToJson = (resolutions) => flatMorph(resolutions, (k, v) => [k, hasArkKind(v, "root") || hasArkKind(v, "generic") ? v.json : hasArkKind(v, "module") ? resolutionsToJson(v) : throwInternalError(`Unexpected resolution ${printable(v)}`)]);
...
var regex$1 = ((src, flags) => new RegExp(src, flags));
...
var preparseKey = (key) => typeof key === "symbol" ? {
	kind: "required",
	normalized: key
...
var parseStandardSchema = (def, ctx) => ctx.$.intrinsic.unknown.pipe((v, ctx) => {
...
var scope = Object.assign(InternalScope.scope, { define: (def) => def });
...
var Merge = genericNode(["base", intrinsic.object], ["props", intrinsic.object])((args) => args.base.merge(args.props), MergeHkt);
...
var liftFrom = genericNode("element")((args) => {
...
var preformattedNodes = flatMorph(normalizedForms, (i, form) => [form, rootSchema({
	domain: "string",
	predicate: (s) => s.normalize(form) === s,
	meta: `${form}-normalized unicode`
})]);
var normalizeNodes = flatMorph(normalizedForms, (i, form) => [form, rootSchema({
	in: "string",
...
var semver = regexStringNode(/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[A-Za-z-][\dA-Za-z-]*)(?:\.(?:0|[1-9]\d*|\d*[A-Za-z-][\dA-Za-z-]*))*))?(?:\+([\dA-Za-z-]+(?:\.[\dA-Za-z-]+)*))?$/, "a semantic version (see https://semver.org/)");
...
var Record = genericNode(["K", intrinsic.key], "V")((args) => ({
	domain: "object",
	index: {
		signature: args.K,
		value: args.V
